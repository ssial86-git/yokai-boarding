# CLAUDE.md — 요괴 하숙집 (가제) (단면 뷰 경영 시뮬, Godot 4.7)

이 파일은 Claude Code가 이 저장소에서 작업할 때 항상 따르는 규약이다.
**컨셉**은 `docs/00_concept.md`, **목표(무엇을 만드는가)**는 `docs/01_vertical_slice_scope.md`, **아트 방침**은 `docs/02_art_sourcing_strategy.md`, **서사·로스터·심사**는 `docs/05~07`이 진실 공급원이다. 이 파일은 **어떻게 작업하는가**만 다룬다. 둘이 충돌하면 작업을 멈추고 사용자에게 알린다.

---

## 1. 프로젝트 한 줄 요약

마계의 폭군을 피해 도망쳐온 요괴들을 받아들여 하숙집을 키우는 단면 뷰 경영 시뮬레이션 (프리미엄 PC — 방치형·광고 훅 금지). 현재 단계는 **수직 슬라이스**이며 스코프는 `docs/01` 3절의 수치로 고정되어 있다. 수치를 넘는 기능 요청이 오면 구현 전에 스코프 문서 개정 여부를 먼저 묻는다.
...
모든 명령은 **프로젝트 루트에서** 실행한다 (gdUnit4 래퍼가 `--path .` 를 쓴다).
`GODOT_BIN` 은 사용자 환경변수로 영구 등록하는 것을 권장한다. 등록돼 있지 않으면 세션마다 첫 줄을 먼저 실행한다.

```powershell
# (미등록 시) 세션 한정 설정
$env:GODOT_BIN = "C:\dev\GODOT\Godot_v4.7-stable_win64_console.exe"

# 에셋 임포트만 (씬 열지 않고 .import / .uid 갱신). 스크립트·에셋을 추가했으면 테스트 전에 한 번 돌린다
& $env:GODOT_BIN --headless --path . --import

# 게임 headless 실행 (스모크: 메인 씬 로드 후 N프레임 뒤 종료)
& $env:GODOT_BIN --headless --path . --quit-after 300

# 단위·통합 테스트 전체 (gdUnit4 v6.2.1 CLI 래퍼 — 내부에서 res://addons/gdUnit4/bin/GdUnitCmdTool.gd 실행)
# 래퍼는 GODOT_BIN 환경변수 또는 --godot_binary <경로> 인자를 요구한다. 둘 다 없으면 종료 코드 1.
# 종료 코드: 0 통과 / 100 실패 / 101 경고 / 103 headless 거부. 보고서: reports/ (git 무시)
# 래퍼는 창을 띄운다(--headless 아님). headless 로 돌리려면 CI 와 같은 직접 호출 형태를 쓴다:
...
# 임시 스프라이트 재생성 (yokai/guest_species/rooms CSV → assets/art_generated/)
python tools/art/gen_placeholder.py
...
CI(`.github/workflows/ci.yml`)도 같은 순서로 돈다.
...
assets/
  ...
  audio/
data/
  csv/                    # ★ 콘텐츠 진실 공급원: yokai.csv(장기 하숙생), guest_species.csv(뜨내기 종족), spirits.csv(가택신), rooms.csv, visitors.csv, rumors.csv, items.csv, recipes.csv, dialogue.csv, events.csv, tuning.csv, strings_ko.csv
  resources/              # build_resources.py 가 생성한 .tres (수동 편집 금지)
src/
  autoload/               # Events.gd(이벤트 버스), GameState.gd, DataRegistry.gd, Clock.gd(하루 페이즈), SaveManager.gd
  ...
  house/                  # 단면 하숙집 씬: 방 슬롯 그리드, 증축 UI, 방 내부
...
docs/
  ...
  decisions/              # ADR: 날짜_제목.md (결정과 이유만 짧게). CSV 스키마는 2026-09-02_csv_schema_v1.md
  asset_licenses.md
```
...
- 런타임은 `DataRegistry` autoload를 통해서만 데이터에 접근한다. `data/csv/`·`tools/`·`docs/`에는 `.gdignore`가 있어 Godot이 스캔하지 않는다 (CSV가 번역 리소스로 임포트되는 것을 막음).
...
- `class_name` 부여(단, `src/autoload/` 스크립트는 autoload 이름과의 충돌을 피하려 붙이지 않는다), 파일명은 snake_case, 클래스명은 PascalCase, 시그널은 과거형 동사(`item_added`), 상수는 UPPER_SNAKE.
...
- 요괴 이동은 방 그래프 탐색(계단·복도 연결)으로 처리한다. 내비메시·물리 이동을 도입하지 않는다.
- 직렬화: 방 배열 + 요괴 상태 + 인벤토리 + 호감도 + 이벤트 로그.
...
- 플레이 감(배치 UI의 드래그 반응·피드백, 하루 템포)은 테스트로 검증할 수 없다. 이 영역은 수치를 `tuning.csv`에 노출하고 사용자에게 "직접 플레이 후 수치 조정 요청"을 명시적으로 넘긴다. 감에 대한 판단을 Claude가 내리지 않는다.
- 새 콘텐츠 추가 = CSV 행 추가로 끝나는지가 상시 검증 대상이다 (Go/No-Go 기준 3번).
