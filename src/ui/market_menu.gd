...
extends ListMenu
## 회색 시장 메뉴 (P2-S3): 위에 사기(하루 재고·오늘 구매가), 아래 팔기(오늘 시세와 대문간 대비). 규칙은 MarketSystem.
...
func open() -> void:
	open_with_title(DataRegistry.text("ui_market_title", {
...
func refresh() -> void:
	...
	for row in market_system.buyable_rows():
		var name := DataRegistry.item_name(row.item_id)
		var left := market_system.stock_left(row.item_id)
		if left <= 0:
			add_row(DataRegistry.text("ui_market_sold_out", {"name": name}))
			continue
		var price := market_system.buy_price(row.item_id)
		var item_id := row.item_id
		add_row(DataRegistry.text("ui_market_buy_row", {"name": name, "price": price, "stock": left}),
			DataRegistry.text("ui_market_buy"), func() -> void: market_system.buy(item_id), GameState.money >= price)
	...
	for item_id in ids:
		...
		var price := market_system.sell_price(item_id)
		...
		add_row(text, DataRegistry.text("ui_sell_all", {"name": DataRegistry.item_name(item_id), "total": total}),
			func() -> void: market_system.sell(id, GameState.inventory.get_count(id)))
