...
func test_p2_blessing_market_tables_loaded_and_typed() -> void:
	...
	assert_int(market.npcs.size()).is_equal(1)
	assert_str(str(market.npcs[0])).is_equal("gray_merchant:gray:300")
	...
	assert_str(DataRegistry.get_event("ev_gray_merchant_greet").kind).is_equal("npc")
