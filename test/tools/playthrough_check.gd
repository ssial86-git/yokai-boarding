...
func _initialize() -> void:
	...
	_check("merchant_present", market_view.get_node_or_null("Npc_gray_merchant") != null, "회색 장꾼 자리표시가 있다")
	...
	# --- P3-S2: 달무리 늪 (회색 시장 너머) + 마을 상점가 (낮) ---
	for unlock_id in ["u_moon_marsh", "u_village"]:
		(gs.get("unlocked") as Dictionary)[unlock_id] = int(gs.get("day"))
	_check("travel_moon_marsh", bool(region_manager.call("travel", "r_moon_marsh")), "회색 시장 → 달무리 늪")
	var marsh_expedition: Node = main.get("expedition_system")
	await _wait_until(func() -> bool: return (marsh_expedition.get("enemies") as Array).size() >= 4, 2.0)
	_check("marsh_enemies", (marsh_expedition.get("enemies") as Array).size() == 4, "달무리 늪에 적 3 + 늪어미")
	_check("marsh_fishing_spot", region_manager.call("current_view").get_node_or_null("FishingSpot") != null, "늪에 낚시 자리")
	await _shot("22_moon_marsh")
	_check("travel_village", bool(region_manager.call("travel", "r_yard")) and bool(region_manager.call("travel", "r_village")), "→ 마당 → 마을 상점가")
	await _frames(2)
	var village_view: Node = region_manager.call("current_view")
	_check("village_npcs", village_view.get_node_or_null("Npc_village_grocer") != null and village_view.get_node_or_null("Npc_herb_granny") != null,
		"마을 잡화상·약방 할미 자리표시 2")
	market_menu.call("open", "village", "village_grocer")
	await _frames(2)
	_check("village_shop_rows", market_menu.visible and int(market_menu.call("row_count")) >= 10, "마을 잡화점 메뉴에 씨앗·곡식 줄")
	await _shot("23_village_shop")
	market_menu.call("close")
	_check("travel_home_from_market", bool(region_manager.call("travel", "r_house")), "마을 → 집")
	await _frames(2)
